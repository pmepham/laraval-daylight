...
<div id="assessment_framework_question_{{ $encQuestionId }}" class="m-0">
    ...
    <div class="bg-hover-light mx-n5 px-5">
        ...
        <div id="{{ $questionForId }}" class="fs-6 ms-1 collapse show">
            <div class="text-gray-600 fw-semibold fs-6 ps-10">
                ...
                @php
                    ...
                    $optionName = 'question['.$encQuestionId.']';
                    $answer = $assessment_answers[$assessment_framework_option['id']] ?? false;
                    $answer = !empty($answer) ? 'checked': '';
                @endphp
                <div class="d-flex flex-column pb-5">
                    <div class="form-check form-check-custom form-check-solid mb-0">
                        <input type="hidden" name="{{ $optionName }}" value="0" />
                        <input class="form-check-input" type="radio" name="{{ $optionName }}" value="{{ $encOptionId }}" id="{{ $optionForId }}" {{ $answer }}/>
                        <label class="form-check-label" for="{{ $optionForId }}">{{ $assessment_framework_option['name'] }}</label>
                    </div>
                </div>
                @endforeach
            </div>
            <div class="d-flex flex-stack pb-5">
                <button class="btn btn-sm btn-primary edit_assessment_framework_question" data-assessment-framework-id="{{ $encFrameworkId }}" data-assessment-framework-question-id="{{ $encQuestionId }}">Edit Question</button>
                <button class="btn btn-sm btn-danger delete_assessment_framework_question"  data-assessment-framework-id="{{ $encFrameworkId }}" data-assessment-framework-question-id="{{ $encQuestionId }}">Delete Question</button>
            </div>
        </div>
    </div>
</div>